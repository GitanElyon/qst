...
qst is the host runtime. Scripts live in `~/.config/qst/scripts/`. Scripts can define custom triggers, query handling, and output formatting via a simple line-oriented protocol. 

Note: qst bundles a helper script, `loader.sh`, and will install it into `~/.config/qst/scripts/` on first run so users can browse and install community scripts without manually copying files.
...
The script ecosystem is cataloged in `awesome-qst`:
...
```bash
qst --config <path>       # points Qst to a different config file
qst --prefill <string>    # seeds the initial search text
qst --shy                 # opens launcher with text hidden until query is entered
qst --no-fuzzy            # disables fuzzy finding
qst --clear-history       # clears Qst's launch history
qst --clear-favorites     # clears favorite list
qst -p, --program <name>  # launches the first result of the query
qst -s, --script <name>   # opens the script on startup
qst --list-programs       # lists available programs
qst --list-scripts        # lists available scripts
qst --gen-config          # generates the default config file
qst --debug-overlay       # starts with the debug overlay visible
qst --log-level <level>   # sets log level: debug, info, warn, error (default: info)
qst -v, --version         # prints the version
qst -h, --help            # prints this help message
```

Or bind to a global hotkey (e.g. `Super+Space`) using your desktop environment's keyboard settings.

Example for Hyprland users to mimic `rofi`:
```
